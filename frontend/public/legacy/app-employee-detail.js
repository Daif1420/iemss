(() => {
  ...
  (async () => {
    try {
      ...
      $('detail-perf').innerHTML = `<div class="info-item"><span>إجمالي التارجت</span><b>${Number.isFinite(totalTarget) ? n(totalTarget) : '—'}</b></div><div class="info-item"><span>نسبة التارجت</span><b>${coloredPercent(s.percentage)}</b></div><div class="info-item"><span>أيام الحضور</span><b>${n(s.total_present_days ?? a.present_days)}</b></div><div class="info-item"><span>إجمالي الغياب</span><b>${n(s.total_absence)}</b></div><div class="info-item"><span>الإضافي</span><b>${hh(s.overtime_hours)}</b></div><div class="info-item"><span>التأخيرات</span><b>${hh(s.late_hours)}</b></div>`;
      ...
      $('detail-head').innerHTML = '<th>المرحلة</th>' + dates.map(x => '<th>' + esc(String(x).slice(5)) + '</th>').join('') + '<th>الإجمالي</th><th>النسبة</th>';
      ...
      const rowsHtml = stageEntries.map(([st, rs]) => {
        ...
        const stagePercent = !isAttendance && hasNum && Number.isFinite(totalTarget) && totalTarget > 0 ? coloredPercent(sum / totalTarget) : '—';
        return '<tr><td>' + esc(st) + '</td>' + cells + '<td>' + (hasNum ? n(sum) : '—') + '</td><td>' + stagePercent + '</td></tr>';
      });
      ...
      if (totalTargetEntry) {
        ...
        rowsHtml.push('<tr class="total-target-row"><td><b>إجمالي التارجت</b></td>' + cells + '<td><b>' + (Number.isFinite(totalTarget) ? n(totalTarget) : '—') + '</b></td><td>' + coloredPercent(s.percentage) + '</td></tr>');
      }

      $('detail-body').innerHTML = rowsHtml.join('') || '<tr><td colspan="' + (dates.length + 3) + '"><div class="empty-state">لا توجد بيانات مطابقة.</div></td></tr>';
    } catch (err) {
